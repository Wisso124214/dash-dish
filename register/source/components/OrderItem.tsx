...
export function OrderItem({ order, menu, selected }: { order: Order, menu: Dish[], selected: boolean }) {
  return (
    <Box flexDirection="column" borderStyle={"round"} padding={1} marginBottom={1} borderColor={selected ? "blue" : "default"}>
      ...
      {order.items.map((item, index) => (
        <Box key={index} flexDirection="column" marginLeft={2} marginBottom={1}>
          <Text>
            Dish: {menu.find(dish => dish._id === item.id_dish)?.title ?? "Unknown"} | Quantity: {item.quantity}
          </Text>
